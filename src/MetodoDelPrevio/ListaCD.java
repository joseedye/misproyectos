...
public class ListaCD<T> implements Iterable<T> {
     ...
     * sirve como cabezaecera de la ListaCD<T>. <br>
     ...
     * <b>post: </b> Elimina todos los datos que contenga la lista circular
     * doble.<br>
     */
    public void vaciar() {
        this.cabeza = new NodoD<T>(null, null, null);
     ...
     * <b>post: </b> Se retorno el elemento indicado por la posicion recibida
     * i.<br>
     *
     ...
     * <b>post: </b> Se retorno true si el elemento se encuentra en la
     * Lista.<br>
     *
     ...
     * Lista Circular Doble en un String. <br>
     * <b>post: </b> Retorna la impresion de los datos de la lista Circular
     * Doble en un String. <br>
     * El String tiene el formato "e1->e2->e3..->en", donde e1, e2, ..., en son
     * los los elementos de la Lista Circular Doble. <br>
     *
     ...
     * objetos que se almacenan en la lista deben tener el Método equals. <br>
     * <b>post: </b> Retorna el Nodo que se encuentra en esa posicion indicada.
     * <br>
     *
    ...
    public int getIndice(T dato) {
     ...
     public void insertarDespuesDe(ListaCD <T>l2, T Info) {
      int taman = this.tamanio+l2.tamanio; 
      int tam = 0;
        for (NodoD<T> x = this.cabeza.getSig(); x != this.cabeza; x = x.getSig()) {
            tam++;
            ...
            if (x.getInfo().equals(Info)) {
                ...
                NodoD<T> segParte = x.getSig();
                NodoD<T> ultimo = this.cabeza.getAnt();
                this.cabeza.setAnt(x);
                x.setSig(this.cabeza);
                
                this.tamanio = tam;
                
                for (NodoD<T> k = l2.cabeza.getSig(); k != l2.cabeza; k = k.getSig()) {
                   
                    this.insertarAlFinal(k.getInfo());
                    if (k.getSig() == l2.cabeza) {
                        segParte.setAnt(this.cabeza.getAnt());
                        this.cabeza.getAnt().setSig(segParte);
                        ultimo.setSig( this.cabeza);
                        this.cabeza.setAnt(ultimo);
                        this.tamanio=taman;
